...
import { validateVideoImageReferences } from '@/utils/videoPrompt'
...
function validateCommon(prompt: string, files: File[], maxFiles = 5): string[] {
  ...
  if (unicodeLength(prompt) > 10000) errors.push('提示词不能超过 10000 字符')
...
export function validateVideoTask(task: VideoTask): string[] {
  ...
  errors.push(...validateVideoImageReferences(task.prompt, task.files.length))
  return errors
}
